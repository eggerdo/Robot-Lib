...
import robots.gui.rover.rover2.Rover2Robot;
...
public class CommandEncoder {
	...
	private static final int HEAD_LEN = 23;

	private static final String OP_HEADER = "MO_O";
	...
	private static final int VIDEO_START_REQ_LEN = 4;
	
	private static final int VIDEO_START_RESP = 5;
	
	private static final int VIDEO_END = 6;
	private static final int VIDEO_END_LEN = 0;
	
	private static final int VIDEO_FRAMEINTERVAL = 7;
	
	private static final int FETCH_BATTERY_POWER_REQ = 251;
	private static final int FETCH_BATTERY_POWER_REQ_LEN = 0;
	
	private static final int FETCH_BATTERY_POWER_RESP = 252;
	...
	private static final int AUDIO_START_REQ = 8;
	...
	private static final int CONTENT_LENGTH_IDX = 15;
	...
	public static ByteArrayBuffer parseCommand(Rover2Controller controller, ByteArrayBuffer arrayBuffer) throws IOException 
	{
		...
		try {
			response = new CommandEncoder().new Protocol(buffer);
		...
		case VIDEO_START_RESP:
			...
			//	      parseAudioStartResp(controller, response);
			//	      return paramByteArrayBuffer;
			//	    case TALK_START_RESP:
			//	      parseTalkStartResp(controller, response);
			//	      return paramByteArrayBuffer;
		case FETCH_BATTERY_POWER_RESP:
			parseFetchBatteryPowerResp(controller, response);
			return arrayBuffer;
		}
	}

	public static void parseFetchBatteryPowerResp(Rover2Controller controller, Protocol response) throws IOException
	{
	...
	public static boolean parseLoginResp(Rover2Controller controller, Protocol response)
	{
		try	{
			...
			if (content.getShort(0) != 0)
				return false;

			// get camera ID
			byte[] cameraBuffer = new byte[CAMERA_ID_LEN];
			content.position(CAMERA_ID_IDX);
			...
			for (int i = 0; i < 4; ++i) {
				device[i] = content.get(i + DEVICE_ID_IDX);
			}
			...
			for (int i = 0; i < 4; ++i) {
				challenge_verify[i] = content.getInt(i * 4 + CHALLENGE_VERIFY_IDX);
			}

			// get reverse challenge
			content.order(ByteOrder.LITTLE_ENDIAN);
			int[] reverse_challenge = new int[4];
			for (int i = 0; i < 4; ++i) {
				reverse_challenge[i] = content.getInt(i * 4 + REVERSE_CHALLENGE_IDX);
			}
			controller.setReverseChallenge(reverse_challenge);

			// check if challenge verification is correct
			...
			Blowfish localBlowFish = new Blowfish(P, S);
			localBlowFish.InitializeBlowfish(controller.getKey().getBytes(), controller.getKey().length());
			localBlowFish.Blowfish_encipher(arr1, arr2);
			localBlowFish.Blowfish_encipher(arr3, arr4);

			String challenge_1 = Utils.intToHex(arr1[0]);
			String challenge_2 = Utils.intToHex(arr2[0]);
			String challenge_3 = Utils.intToHex(arr3[0]);
			String challenge_4 = Utils.intToHex(arr4[0]);

			String verify_1 = Utils.intToReverseHex(challenge_verify[0]);
			String verify_2 = Utils.intToReverseHex(challenge_verify[1]);
			String verify_3 = Utils.intToReverseHex(challenge_verify[2]);
			String verify_4 = Utils.intToReverseHex(challenge_verify[3]);
	...
	public static void parseVerifyResp(Rover2Controller controller, Protocol response)
	{
	...
	public static void parseVideoStartResp(Rover2Controller controller, Protocol response) throws IOException
	{
		int nCameraId = response.getContent().getInt(CAMERA_ID_IDX);
		Log.d(TAG, "cmdVideoStartResp:" + nCameraId);
	...
	{
		Log.d(TAG, "keepAlive");
		return new CommandEncoder().new Command(KEEP_ALIVE, KEEP_ALIVE_LEN).output();
	}

	public static byte[] cmdFetchBatteryPowerReq() throws IOException 
	{
		Log.d(TAG, "fetchBatteryPower");
		return new CommandEncoder().new Command(FETCH_BATTERY_POWER_REQ, FETCH_BATTERY_POWER_REQ_LEN).output();
	}
	
	public static byte[] cmdDecoderControlReq(int value) throws IOException 
	{
		Log.d(TAG, "decoderControlReq");
		Command cmd = new CommandEncoder().new Command(DECODER_CONTROL_REQ, DECODER_CONTROL_REQ_LEN);
	...
	{
		Log.d(TAG, "deviceControlReq");
		Command cmd = new CommandEncoder().new Command(DEVICE_CONTROL_REQ, DEVICE_CONTROL_REQ_LEN);
	...
	{
		Log.d(TAG, "loginReq");
		Command cmd = new CommandEncoder().new Command(LOGIN_REQ, LOGIN_REQ_LEN);
	...
	public static byte[] cmdVerifyReq(Rover2Controller controller, String paramString, int[] reverse_challenge) throws IOException 
	{
		...
		Command cmd = new CommandEncoder().new Command(VERIFY_REQ, VERIFY_REQ_LEN);
		ByteBuffer content = cmd.getContent();
		content.putInt(arrayOfInt1[0]);
		content.putInt(arrayOfInt2[0]);
		content.putInt(arrayOfInt3[0]);
		content.putInt(arrayOfInt4[0]);
	...
	public static byte[] cmdMediaLoginReq(int nID) throws IOException 
	{
		...
		Media media = new CommandEncoder().new Media(MEDIA_LOGIN_REQ, MEDIA_LOGIN_REQ_LEN);
		media.getContent().putInt(nID);
		return media.output();
	}
	...
	public static ByteArrayBuffer parseMediaCommand(Rover2Controller controller, ByteArrayBuffer arrayBuffer) throws IOException
	{
		...
		case VIDEO_DATA:
			...
			break;
		case AUDIO_DATA:
			//	      parseAudioData(controller, response);
			break;
		default:
			Log.d(TAG, "default");
		}
	...
	public static void parseVideoData(Rover2Controller controller, Protocol response)
	{
		ByteBuffer content = response.getContent();
		int dataLength = content.getInt(VIDEO_DATA_LEN_IDX);
	...
	{
		Log.d(TAG, "videEnd");
		return new CommandEncoder().new Command(VIDEO_END, VIDEO_END_LEN).output();
	}

	public static byte[] cmdVideoStartReq() throws IOException
	{
		Log.d(TAG, "cmdVideoStartReq");
		Command cmd = new CommandEncoder().new Command(VIDEO_START_REQ, VIDEO_START_REQ_LEN);
	...
	private class Protocol
	{
		...
		public Protocol(byte[] buffer, int offset) {

			ByteBuffer bb = ByteBuffer.wrap(buffer);
			bb.order(ByteOrder.LITTLE_ENDIAN);
			if (bb.capacity() > HEAD_LEN)
			{
				this.contentLength = bb.getInt(CONTENT_LENGTH_IDX + offset);
				if (bb.capacity() < contentLength + HEAD_LEN)
					throw new BufferUnderflowException();
			} else
				throw new BufferUnderflowException();

			bb.position(HEADER_IDX + offset);
			bb.get(this.header);

			this.opCode = bb.getShort(OP_IDX + offset);

			this.content = ByteBuffer.allocate(this.contentLength);
			this.content.order(ByteOrder.LITTLE_ENDIAN);
			bb.position(HEAD_LEN + offset);
			bb.get(content.array());
		}
		
		public Protocol(String header, int op_code, int content_length) {
		...
		public int size() {
			return this.contentLength + HEAD_LEN;
		}

		public byte[] output() throws IOException 
		{
			ByteBuffer bb = ByteBuffer.allocate(HEAD_LEN + this.contentLength);
			bb.order(ByteOrder.LITTLE_ENDIAN);
			bb.put(header);
	...
	private class Command extends Protocol {

		public Command(int op_code, int content_length) {
			super(OP_HEADER, op_code, content_length);
		}

	}

	private class Media extends Protocol {

		public Media(int op_code, int content_length) {
			super(MEDIA_OP_HEADER, op_code, content_length);
		}
