 ...
 * we had to add SubMoves to distinguish between left, straight and right while
...
public class RoboScooper extends BaseRobot {
	...
	private SubMove radiusToSubMove(int i_nRadius) {
		if (i_nRadius > 30) {
			return SubMove.LEFT;
		} else if (i_nRadius < -30) {
			return SubMove.RIGHT;
		} else {
			return SubMove.STRAIGHT;
		}
	}

	private void startMove(Move i_eMove, int i_nRadius) {
		// convert the radius to the SubMoves LEFT, RIGHT, STRAIGHT
		SubMove eCmd = radiusToSubMove(i_nRadius);
	...
	public void moveForward(double i_dblSpeed, int i_nRadius) {
		...
		startMove(Move.FORWARD, i_nRadius);
	}
	...
	public void moveBackward(double i_dblSpeed, int i_nRadius) {
		...
		startMove(Move.BACKWARD, i_nRadius);
	}
