...
//		byte[] result = new byte[nLength];
	...
	// receiving is done blocking/synchronous because the messages
	// sent by the roomba don't have an identification, so we can only handle
	// a response if we know what the request was
	private boolean sendCmdAndCheckReply(String i_strCmd, String i_strReply) {
		...
		while (true) {
			if (m_oConnection.isDataAvailable()) {
				m_oConnection.read(reply, 0, reply.length);
				break;
			} else {
	...
	private byte[] getResults(int i_nBytes) throws TimeoutException {
		...
		while (nReceivedBytes != nRequiredBytes) {
			if (m_oConnection.isDataAvailable()) {
				nReceivedBytes += m_oConnection.read(buffer, nReceivedBytes, nRequiredBytes - nReceivedBytes);
			} else {
				Utils.waitSomeTime(1);
				if (System.currentTimeMillis() - start > 500) {
